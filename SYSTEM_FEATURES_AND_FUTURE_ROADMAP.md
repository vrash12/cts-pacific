...
- Hosted image delivery: public marketing images use prepared responsive files through `SiteImage`, with hashed original/variant URLs and immutable image caching. The image preparation script runs during dev startup, tests, and production builds; generated delivery files are ignored by Git and rebuilt on the host. Hero and header-logo preloads are present in initial HTML, logo size hints match the rendered width, and first-screen entrance delays are removed. Unknown/future image sources retain Next's standard optimizer. See `docs/architecture/IMAGE_DELIVERY.md` for architecture and verification.
- Image-delivery validation: lint, typechecking, 95 unit/integration tests, and production build passed. The full production-browser suite passed 113 checks with one intentional mobile skip. Cold-browser throttling showed desktop homepage hero readiness improving from 3.83 s to 2.77 s; results for all sampled pages and network conditions are recorded in the image-delivery document. No production timing guarantee is inferred from local samples.
- Micro Trenching directory card now uses the user's separately supplied September 6 Vermeer cutting-wheel photo, replacing the stock pavement-saw reference. Nine relevant image tests, production build, and desktop/mobile card review passed; source provenance is in `CLIENT_PHOTOGRAPHY.md`.
- Live hosting follow-up: `ctspacific.com` identified itself as Hostinger and was still serving the older `/_next/image` markup after the performance commit was pushed. Browser checks reproduced intermittent HTTP/2 and QUIC protocol failures on those live optimizer requests. GitHub CI succeeded, but live deployment of the new image-delivery code has not been confirmed. Verify Hostinger's deployed branch/commit and rebuild the latest `main`; do not report the live image issue resolved solely because local tests pass.
